...
4. **RESOLVED (Prompt 3): the PRD's 100 bps example was unachievable** (measured 251 bps now, about 351 bps after the transfer fee rises at Solana epoch 1039). PRD example corrected. The mid-mandate fee change is an open product consideration (current-market.md).
5. **Not yet done, deliberately:** Cross-OS (Linux) determinism has not been run locally (Docker daemon not running); the CI job covers it. Surfpool was installed but a fork has not been started (Prompt 13 uses it; a smoke start belongs with Prompt 3 measurement); Clawpump `/pump-pairs` and its docs were not verified because no credentials exist and it is gated; Solana developer MCP and Meteora docs MCP are not registered in this environment, so the Meteora docs were read directly over HTTPS. Meteora "DLMM Formulas", "Collect Fee Mode" and the API reference will be read at the start of Prompt 3.
